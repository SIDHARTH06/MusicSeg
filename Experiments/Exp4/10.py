...
class MusicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_labels = []
        for i in range(39):  # 13 segments
            start_boundaries = np.where((start_times >= i * 10) & (start_times < (i + 1) * 10))[0]
            stop_boundaries = np.where((stop_times >= i * 10) & (stop_times < (i + 1) * 10))[0]
...
# Example usage:
mfcc_dir = "../../Dataset B/Processed/MFCC/10"
...
hidden_size = 64
num_layers = 1
output_size = 39  # Number of LSTM units (one for each window)
...
with open('./result/res_10.txt', 'w') as f:
    ...
    plt.show()
    # Save the figure
    plt.savefig(f'result/precision_recall_curve_10_{fold}.png')
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('result/evaluation_metrics_10.png')
    plt.show()
