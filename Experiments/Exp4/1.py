...
class MusicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_labels = []
        for i in range(392):  # 13 segments
...
# Example usage:
mfcc_dir = "../../Dataset B/Processed/MFCC/1"
...
hidden_size = 64
num_layers = 1
output_size = 392  # Number of LSTM units (one for each window)
...
with open('./result/res_1.txt', 'w') as f:
    ...
    plt.show()
    # Save the figure
    plt.savefig(f'result/precision_recall_curve_1_{fold}.png')
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig('result/evaluation_metrics_1.png')
    plt.show()
