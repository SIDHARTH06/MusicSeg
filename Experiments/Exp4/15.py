...
# Example usage:
mfcc_dir = "../../Dataset A/Processed/MFCC/15swindow"
...
hidden_size = 64
num_layers = 1
output_size = 27  # Number of LSTM units (one for each window)
...
with open('results/res_15.txt', 'w') as f:
...
for fold, (train_index, val_index) in enumerate(kf.split(train_set)):
    ...
    with open('results/res_15.txt', 'a') as f:
...
plt.grid(True)
plt.savefig('results/precision_recall_curve_15.png')
plt.show()
...
plt.grid(True)
plt.legend()
plt.savefig('results/evaluation_metrics_15.png')
plt.show()

# Save PR curve data
np.save('results/precision_values_15.npy', np.array(precision_values))
np.save('results/recall_values_15.npy', np.array(recall_values))
np.save('results/pr_auc_values_15.npy', np.array(pr_auc_values))
